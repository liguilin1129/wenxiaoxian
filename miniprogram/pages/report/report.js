...
Page({
  data: { dims: [], neigong: [], rules: [], weekGain: 0, points: 0, streak: 0, done: 0, total: 0, completionPct: 0, earned: 0, spent: 0, recordCount: 0, focus: {}, strength: {}, suggestion: '' },
  onShow() {
    ...
    this.setData({ dims: dims, neigong: g.neigong || [], rules: store.getScoreRules(), weekGain: g.child.weekGain || 0, points: g.child.points || 0, streak: g.child.streak || 0, done: done, total: today.length, completionPct: today.length ? Math.round(done / today.length * 100) : 0, earned: earned, spent: spent, recordCount: history.length, focus: focus, strength: strength, suggestion: suggestion });
  }
});
